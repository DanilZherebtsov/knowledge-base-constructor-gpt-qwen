...
THRESHOLD_DAYS = 7  # keep in sync with QWEN.md discipline §5 and STATE freshness trigger
...
def project_root() -> str:
    """Walk up from the session cwd to the directory holding QWEN.md.
    ...
    d = start
    while True:
        if os.path.isfile(os.path.join(d, "QWEN.md")):
            return d
...
def live_tmp_runs(root: str, limit: int = 3):
    """Subfolders of tmp/ — one per long-pass run (see QWEN.md, `tmp/`)."""
